...
void main()                         
{
   ...
   verticeToLight = -normalize(lightPosition.xyz);
   ...
   vec3 vTexCoords = reflect(v, finalNormal);
   vec4 reflectColor = textureCube(cube, vTexCoords);

   gl_FragColor = ambientColor*reflectColor + vDiffuse*reflectColor + vSpecular;
}